...
def plot(data, x_observable, y_observables, zero_x_axis, zero_y_axis):
    fig, ax = plt.subplots(layout="constrained")
    if not y_observables:
        y_observables.append("m_v")

    x_name, x_label = parse_observable(x_observable)

    markers = "os^v"
    for color_index, (y_observable, marker) in enumerate(zip(y_observables, markers)):
        y_name, y_label = parse_observable(y_observable)
        errorbar_pyerrors(
            ax,
            data[x_name],
            data[y_name],
    ...
    ax.set_ylabel(get_ylabel(y_observables))
